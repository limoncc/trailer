...
- **3D** — 可交互 Three.js 曲面:拖拽旋转、滚轮缩放、线框开关、Front/Side/Top 视角
- **⚽ Roll + 球速** — **三种视图**都可重放梯度下降小球(从最高点滚落,带尾迹);速度 0.5×–4× 可调;切换视图或新数据帧时自动播放
- **配色选择器** — coolwarm(默认)/ plasma / magma / viridis,热力图、等高线与 3D 曲面同步应用
- **值域刻度(log / lin)** — 默认 **log**:偏移对数刻度放大碗底细节,极端"墙"不再把 min–max 色阶压成一片平色;等高线分级随之在低区变密;3D 的 loss 轴会标注 `(log)`。悬停 tooltip 始终显示原始 loss

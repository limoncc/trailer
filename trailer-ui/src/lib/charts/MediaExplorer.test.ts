...
describe('MediaExplorer component', () => {
  it('is exported from charts index', async () => {
    const charts = await import('./index.ts');
    expect(charts.MediaExplorer).toBeDefined();
  }, 20000);

  it('has MediaExplorer.svelte component file', async () => {
    const mod = await import('./MediaExplorer.svelte');

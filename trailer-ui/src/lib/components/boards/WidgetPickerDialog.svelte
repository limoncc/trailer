<script lang="ts">
  ...
  let selectedName = $state(
    initWidget &&
      (initWidget.type === 'figure' || initWidget.type === 'pca' || initWidget.type === 'landscape' || initWidget.type === 'text')
      ? initWidget.name
      : ''
  );
  ...
  const landscapeNames = $derived(uniqueNames(boardsData.figures.filter((f) => f.kind === 'landscape')));
...
<div class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-card border border-border rounded-xl shadow-xl w-[560px] max-w-[92vw] max-h-[80vh] flex flex-col">
    ...
    <div class="flex-1 overflow-auto px-4 py-3">
      ...
      {:else if activeType === 'landscape'}
        ...
        {/if}
        {#if landscapeNames.length === 0}
          <p class="text-xs text-muted-foreground text-center py-6">Run has no landscape data yet</p>
        {:else}
          <div class="space-y-0.5">
            {#each landscapeNames as f (f.name)}
              <label class="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-accent/50 cursor-pointer text-xs font-mono {selectedName === f.name ? 'bg-accent' : ''}">
                <input type="radio" name="landscape-pick" checked={selectedName === f.name} onchange={() => (selectedName = f.name)} class="accent-primary" />
                <span class="truncate flex-1">{f.name}</span>
                <span class="text-[10px] text-muted-foreground">{f.count} · step {f.latestStep}</span>
              </label>
            {/each}
